...
var BeautifierSettings = /** @class */ (function () {
    function BeautifierSettings(removeComments, removeReport, checkAlias, signAlignSettings, keywordCase, typeNameCase, indentation, newLineSettings, endOfLine) {
...
var TypeNames = ["BOOLEAN", "BIT", "CHARACTER", "INTEGER", "TIME", "NATURAL", "POSITIVE", "STRING"];
...
function beautify3(inputs, result, settings, startIndex, indent, endIndex) {
    ...
    for (i = startIndex; i <= endIndex; i++) {
        ...
        if (input.regexStartsWith(/[\w\s:]*\bPORT\b([\s]|$)/)) {
            _d = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "PORT"), i = _d[0], endIndex = _d[1];
            continue;
        }
        if (input.regexStartsWith(/TYPE\s+\w+\s+IS\s+\(/)) {
            _e = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "IS"), i = _e[0], endIndex = _e[1];
            continue;
        }
        if (input.regexStartsWith(/[\w\s:]*GENERIC([\s]|$)/)) {
            _f = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "GENERIC"), i = _f[0], endIndex = _f[1];
            continue;
        }
        if (input.regexStartsWith(/[\w\s:]*PROCEDURE[\s\w]+\($/)) {
            _g = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "PROCEDURE"), i = _g[0], endIndex = _g[1];
        ...
        if (input.regexStartsWith(/FUNCTION[^\w]/)
            ...
            _h = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "FUNCTION"), i = _h[0], endIndex = _h[1];
        ...
        if (input.regexStartsWith(/IMPURE FUNCTION[^\w]/)
            && input.regexIndexOf(/[^\w]RETURN[^\w]/) < 0) {
            _j = beautifyPortGenericBlock(inputs, result, settings, i, endIndex, indent, "IMPURE FUNCTION"), i = _j[0], endIndex = _j[1];
            if (!inputs[i].regexStartsWith(regexBlockEndsKeyWords)) {
